...
namespace VaultSharp.V1.AuthMethods.Token
{
    internal class TokenAuthMethodProvider : ITokenAuthMethod
    {
        ...
        public async Task<AuthInfo> RenewSelfAsync(string increment = null)
        {
            ...
            var result = await _polymath.MakeVaultApiRequest<Secret<JsonElement>>("v1/auth/token/renew-self", HttpMethod.Post, requestData).ConfigureAwait(_polymath.VaultClientSettings.ContinueAsyncTasksOnCapturedContext);
            return result.AuthInfo;
        }

        public async Task RevokeSelfAsync()
        {
            await _polymath.MakeVaultApiRequest<Secret<JsonElement>>("v1/auth/token/revoke-self", HttpMethod.Post).ConfigureAwait(_polymath.VaultClientSettings.ContinueAsyncTasksOnCapturedContext);
        }
    }
}